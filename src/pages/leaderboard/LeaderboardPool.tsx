...
const LeaderboardPool = () => {
  ...
  return (
    <Layout>
      ...
      <div className="bg-gray-300 p-3 w-full mt-2">
        <div className="flex justify-between items-center">
          <p>{data?.personalRank ? data?.personalRank[0]?.rank || 'No rank' : 'Yet to predict'}</p>
          <div className="flex gap-3 justify-center items-center">
            <div className="p-1 border-black rounded-full border-[1px]">{data?.personalRank ? data.personalRank[0]?.user?.firstName?.split(" ")[0]?.charAt(0) || "Y" : ""}
                    {data?.personalRank ? data.personalRank[0]?.user?.lastName?.split(" ")[1]?.charAt(0) || "O" : ""}</div>
            <p>You</p>
          </div>
